...
class Top_ECP5 extends Component {
  ...
  val clk14Domain = ClockDomain.internal(
    name = "Core14",
    frequency = FixedFrequency(14.3182 MHz)
  )
  val clk11Domain = ClockDomain.internal(
    name = "Core11",
    frequency = FixedFrequency(11.3407 MHz)
  )
  val clk25Domain = ClockDomain.internal(
    name = "Core25",
    frequency = FixedFrequency(25.0000 MHz)
  )
  ...
  val Core11 = new ClockingArea(clk11Domain) {
    ...
    val areaRst = new ResetArea(!reset, false) {
      ...
      val areaDiv4 = new SlowArea(2) {
        ...
        val rom_fdc = new RamInit("./data/fdc.bin", log2Up(0x1fff))

        rom_fdc.io.addra := comx35.io.FDCRom.Addr
        rom_fdc.io.dina := 0
        rom_fdc.io.wea := 0
        rom_fdc.io.ena := True
        ...
        when(!comx35.io.MRD) {
          when(
            comx35.io.ExtRom || (comx35.io.Addr16.asUInt >= 0xc000 && comx35.io.Addr16.asUInt <= 0xdfff) || comx35.io.N === 2
          ) {
            comx35.io.DataIn := comx35.io.Card_DataOut
          } elsewhen (comx35.io.Addr16.asUInt < 0x4000) {
            when(comx35.io.N === 0) {
              // save and loads stop
              ...
              when(
                comx35.io.Addr16 === 0x1672 || comx35.io.Addr16 === 0x1675 || comx35.io.Addr16 === 0x0e00 || comx35.io.Addr16 === 0x0e03
              ) {
                Tape_start := True
              }
            }
        ...
        when(
          comx35.io.Addr16.asUInt >= 0x4000 && comx35.io.Addr16.asUInt < 0xc000
        ) {
  ...
  val Core25 = new ClockingArea(clk25Domain) {
    ...
    downs.io.i_data := soundBuff.resize(
      8
    ) + (io.tape_led ? S"000001" | S"000000");
